...
export function CartPage() {
  ...
  return (
    <PageShell>
      <PageHeader
        title="Your cart"
        subtitle={restaurant ? `From ${restaurant.name}` : ''}
        back={{ to: `/browse-meals/${cart.restaurantId}`, label: 'Back to menu' }}
      />
      ...
      <div className="cart-layout" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <Card key={item.mealId}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 13, color: '#718096' }}>
                        {fmtMoney(parseFloat(meal.price))} each
                      </div>
                    </div>
                  </div>
                  <div
                  ...
                  >
                    <QtyStepper
                      value={item.qty}
                      onChange={(qty) => updateCartQty(item.mealId, qty)}
                      min={0}
                    />
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <span style={{ fontWeight: 700, fontSize: 15 }}>
                        {fmtMoney(parseFloat(meal.price) * item.qty)}
                      </span>
                      <button
        ...
        <Card>
          ...
          {appliedCoupon && (
            ...
            >
              <span>
                <strong>{appliedCoupon.code}</strong> — {appliedCoupon.discount_percentage}% off
              </span>
              <button
                onClick={() => {
          ...
          {discount > 0 && (
            <SummaryRow label="Discount" value={`−${fmtMoney(discount)}`} valueColor="#38A169" />
          )}
