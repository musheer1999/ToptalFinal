  ...
  return (
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <Field label="Cuisine">
          <Select value={form.cuisine} onChange={set('cuisine')} options={CUISINES} />
        </Field>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          <Field label="Longitude" hint="Optional">
            <Input value={form.longitude} onChange={set('longitude')} placeholder="-122.4194" />
          </Field>
          <Field label="Latitude" hint="Optional">
            <Input value={form.latitude} onChange={set('latitude')} placeholder="37.7749" />
          </Field>
        </div>
      </div>
    </Modal>
    ...
    <PageShell>
      <div style={{ maxWidth: 520, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          ...
          <p style={{ margin: '8px 0 0', fontSize: 14, color: '#718096' }}>
            Welcome, {session?.email}! Set up your restaurant to start receiving orders.
          </p>
        </div>
        <Card>
          <form
