...
function ManageRestaurantInner() {
  ...
  return (
    <PageShell>
      ...
      <Card>
        ...
        {[
          ...
          { label: 'Description', value: ownerRestaurant.description },
          {
            label: 'Longitude',
            value:
              ownerRestaurant.longitude != null
                ? parseFloat(ownerRestaurant.longitude).toFixed(6)
                : '—',
            mono: true,
          },
          {
            label: 'Latitude',
            value:
              ownerRestaurant.latitude != null
                ? parseFloat(ownerRestaurant.latitude).toFixed(6)
                : '—',
            mono: true,
          },
        ].map((row) => (
          ...
          >
            <div style={{ fontSize: 12, color: '#718096', minWidth: 90, flexShrink: 0 }}>
              {row.label}
            </div>
            <div
              style={{
      ...
      {editing && (
        <RestaurantEditModal restaurant={ownerRestaurant} onClose={() => setEditing(false)} />
      )}
    </PageShell>
  );
}
