...
function useOwnerOrders() {
  ...
  const stats = useMemo(() => ([
    ...
    { label: 'New', value: orders.filter((order) => order.status === 'Placed').length, color: '#2563EB' },
    { label: 'Kitchen', value: orders.filter((order) => order.status === 'Processing').length, color: '#FF6B35' },
    { label: 'On way', value: orders.filter((order) => order.status === 'In Route').length, color: '#9333EA' },
    { label: 'Delivered', value: orders.filter((order) => order.status === 'Delivered').length, color: '#0891B2' },
  ]), [orders]);
