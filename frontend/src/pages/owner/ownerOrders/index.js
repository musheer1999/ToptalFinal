...
function OwnerOrdersInner() {
  ...
  if (loading)
    return (
      <PageShell>
        <div style={{ textAlign: 'center', padding: 60, color: '#718096' }}>Loading orders...</div>
      </PageShell>
    );
    ...
    <PageShell>
      ...
      >
        {stats.map((stat) => (
          ...
          >
            ...
            <div style={{ fontSize: 22, fontWeight: 800, color: stat.color, marginTop: 2 }}>
              {stat.value}
            </div>
          </div>
        ))}
      ...
      {orders.length === 0 ? (
        <EmptyState
          icon="📭"
          title="No orders yet"
          body="Orders from customers will appear here."
        />
      ) : (
