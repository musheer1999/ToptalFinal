  ...
  return (
    <Modal
      ...
      footer={
        <>
          ...
          <Button
            onClick={() =>
              onSave({ code: code.toUpperCase(), discount_percentage: parseInt(pct, 10) || 0 })
            }
          >
            Save
          </Button>
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <Field label="Code">
          <Input value={code} onChange={(value) => setCode(value.toUpperCase())} />
        </Field>
        <Field label="Discount %">
          <Input value={pct} onChange={(value) => setPct(value.replace(/\D/g, ''))} />
        </Field>
      </div>
    </Modal>
  );
...
function ManageCouponsInner() {
  ...
  useEffect(() => {
    loadCoupons(ownerRestaurant.id)
      .then(onCouponsLoaded)
      .catch(() => {})
  ...
  const handleCreate = async (event) => {
    ...
    if (Number.isNaN(discount) || discount < 1 || discount > 100)
      return notifyError('Discount must be 1–100');
    try {
      await createCoupon(ownerRestaurant.id, {
  ...
  if (loading)
    return (
      <PageShell>
        <div style={{ textAlign: 'center', padding: 60, color: '#718096' }}>Loading coupons...</div>
      </PageShell>
    );
    ...
    <PageShell>
      ...
      {showForm && (
        <Card style={{ marginBottom: 16 }}>
          ...
          >
            ...
            <Field label="Discount %" hint="1 to 100">
              <Input
                value={pct}
                onChange={(value) => setPct(value.replace(/\D/g, ''))}
                placeholder="20"
              />
            </Field>
      ...
      {coupons.length === 0 ? (
        <EmptyState
          ...
          body='Tap "+ Add new coupon" to create a discount.'
        />
      ) : (
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 700, fontSize: 14 }}>
                  <code style={{ background: '#F7F8FC', padding: '2px 8px', borderRadius: 6 }}>
                    {coupon.code}
                  </code>
                </div>
                <div style={{ fontSize: 13, color: '#38A169', fontWeight: 600, marginTop: 2 }}>
                  {coupon.discount_percentage}% off
                </div>
              </div>
              <div style={{ display: 'flex', gap: 6 }}>
                <Button size="sm" variant="secondary" onClick={() => setEditingCoupon(coupon)}>
                  Edit
                </Button>
                <Button
                  size="sm"
      ...
      {editingRest && (
        <RestaurantEditModal restaurant={ownerRestaurant} onClose={() => setEditingRest(false)} />
      )}
    </PageShell>
  );
}
