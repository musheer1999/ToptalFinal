  ...
  return (
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <Field label="Name">
          <Input value={form.name} onChange={setFormField('name')} />
        </Field>
        <Field label="Description">
          <Textarea value={form.description} onChange={setFormField('description')} rows={2} />
        </Field>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          <Field label="Price (₹)">
            <Input
              value={form.price}
              onChange={(value) =>
                setForm((prev) => ({ ...prev, price: value.replace(/[^0-9.]/g, '') }))
              }
            />
          </Field>
          <Field label="Type">
            <Select value={form.type} onChange={setFormField('type')} options={MEAL_TYPES} />
          </Field>
        </div>
      </div>
    </Modal>
...
function ManageMealsInner() {
  ...
  const handleCreate = async (event) => {
    ...
    try {
      await createMeal(ownerRestaurant.id, {
        ...form,
        name: form.name.trim(),
  ...
  if (loading)
    return (
      <PageShell>
        <div style={{ textAlign: 'center', padding: 60, color: '#718096' }}>Loading meals...</div>
      </PageShell>
    );
    ...
    <PageShell>
      ...
      {meals.length === 0 ? (
        <EmptyState
          icon="🍕"
          title="No meals yet"
          body='Tap "+ Add new meal" to add your first item.'
        />
      ) : (
          ...
          {meals.map((meal) => (
            ...
            >
              ...
              <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                <Button size="sm" variant="secondary" onClick={() => setEditingMeal(meal)}>
                  Edit
                </Button>
                <Button size="sm" variant="danger" onClick={() => setDeletingMeal(meal)}>
                  ✕
                </Button>
              </div>
            </div>
      ...
      {deletingMeal && (
        ...
        >
          <p style={{ margin: 0, color: '#4A5568' }}>
            <strong>{deletingMeal.name}</strong> will be removed.
          </p>
        </Modal>
      )}
      {editingRest && (
