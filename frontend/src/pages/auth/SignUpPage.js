    ...
    <AuthCard title="Create account" subtitle="Join ToptalMeals today">
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        <Field label="Email address" error={errors.email}>
          <Input type="email" value={email} onChange={v => { setEmail(v); setErrors(p => ({...p, email: ''})); }}
            placeholder="you@gmail.com" error={errors.email} />
        </Field>
        <Field label="Password" error={errors.password}>
          <Input type="password" value={password} onChange={v => { setPassword(v); setErrors(p => ({...p, password: ''})); }}
            placeholder="Min. 6 characters" error={errors.password} />
        </Field>
        <Field label="I am a..." error={errors.role}>
          <Select value={role} onChange={v => { setRole(v); setErrors(p => ({...p, role: ''})); }}
