...
function CustomerNavLinks({ onOpenCart }) {
  ...
  const navBtn = (to, fullLabel, shortLabel, active) => (
    <button onClick={() => navigate(to)} style={{
      padding: '6px 8px', borderRadius: 8, fontSize: 13, fontWeight: 500, whiteSpace: 'nowrap',
  ...
  return (
    <nav style={{ display: 'flex', alignItems: 'center', gap: 2, flexShrink: 0 }}>
      {navBtn('/browse-restaurants', 'Restaurants', 'Browse', pathname.startsWith('/browse-restaurants'))}
      ...
      <button onClick={onOpenCart} style={{
        ...
        padding: '6px 10px', display: 'inline-flex', alignItems: 'center', gap: 5,
        fontSize: 13, fontWeight: 600, color: '#1A202C', cursor: 'pointer',
        fontFamily: 'inherit', marginLeft: 2, whiteSpace: 'nowrap', flexShrink: 0,
      }}>
        ...
        {cartCount > 0
          ? <span style={{ background: '#FF6B35', color: 'white', borderRadius: 999, minWidth: 18, height: 18, fontSize: 10, fontWeight: 700, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', padding: '0 4px' }}>{cartCount}</span>
          : null
        }
      </button>
    </nav>
...
function OwnerNavLinks() {
  ...
  const isManageActive = ['/owner/restaurants', '/owner/meals', '/owner/coupons', '/owner/users'].some(p => pathname.startsWith(p));
  ...
  return (
    <nav style={{ display: 'flex', alignItems: 'center', gap: 2, flexShrink: 0 }}>
      ...
      <div ref={ref} style={{ position: 'relative' }}>
        ...
        {open && (
          <div style={{
            position: 'fixed', top: 56, right: 12, // fixed so it doesn't clip
            background: 'white', border: '1px solid #EDF0F5', borderRadius: 12,
            boxShadow: '0 12px 32px rgba(15,23,42,0.14)', minWidth: 190, padding: 6, zIndex: 200,
          }}>
            {[
              ...
              <button key={item.to} onClick={() => { navigate(item.to); setOpen(false); }} style={{
                display: 'flex', alignItems: 'center', gap: 10, width: '100%',
                padding: '10px 12px', borderRadius: 8, fontSize: 14, color: '#1A202C',
    ...
    <div ref={ref} style={{ position: 'relative', flexShrink: 0 }}>
      ...
      {open && (
        <div style={{
          ...
          background: 'white', border: '1px solid #EDF0F5', borderRadius: 12,
          boxShadow: '0 12px 32px rgba(15,23,42,0.14)', minWidth: 200, padding: 6, zIndex: 200,
        }}>
          <div style={{ padding: '8px 12px 2px', fontSize: 11, color: '#718096', textTransform: 'uppercase', letterSpacing: 0.5 }}>Signed in as</div>
          <div style={{ padding: '0 12px 6px', fontSize: 13, fontWeight: 600, wordBreak: 'break-all' }}>{session.email}</div>
          <div style={{ padding: '0 12px 6px', fontSize: 11, color: '#718096', textTransform: 'uppercase', letterSpacing: 0.5 }}>{session.role}</div>
          <div style={{ height: 1, background: '#EDF0F5', margin: '4px 0' }} />
          <button onClick={() => { setOpen(false); signOut(); navigate('/'); }} style={{
            width: '100%', textAlign: 'left', padding: '10px 12px', borderRadius: 8,
            border: 'none', background: 'transparent', cursor: 'pointer',
            fontSize: 14, color: '#E53E3E', fontFamily: 'inherit',
          }}>Sign out</button>
        </div>
      )}
    </div>
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: 16 }}>
          {cartCount === 0 ? (
            <div style={{ textAlign: 'center', padding: '60px 24px' }}>
              ...
              <div style={{ fontSize: 14, color: '#718096', marginTop: 6 }}>Add meals to get started</div>
            </div>
          ) : (
        ...
        {cartCount > 0 && (
          <div style={{ borderTop: '1px solid #EDF0F5', padding: 16 }}>
            ...
            <Button full size="lg" onClick={() => { onClose(); navigate('/cart'); }}>Go to checkout →</Button>
          </div>
        )}
      </div>
    </div>
